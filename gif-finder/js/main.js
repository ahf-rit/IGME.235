...
function searchButtonClicked(){
    ...
    // 3 - build up out URL string
    let url = GIPHY_URL;
    url += "api_key=" + GIPHY_KEY;

    // 4 - parse the user entered term we wish to searth
    ...
    // 7 - if there's no term to search then bal out of the function (return does this)
...
function dataLoaded(e) {
    ...
    // 6 - xhr.responseText is the JSON file we just donwloaded
    console.log(xhr.responseText);

    // 7 - turn the text into a parsable JavaScript object
    let obj = JSON.parse(xhr.responseText);

    // 8 - if there are nor esults, print a message and return
